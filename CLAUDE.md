...
**GPS-tick anchored slicing**: the at-window is anchored by finding the GPS row whose UTC is closest to the requested wall-clock time and using its tick directly, rather than linearly extrapolating from the first GPS sample. ThreadX runs on HSI (internal RC, ±1 % accuracy) and drifts ~7 s over a 21-min session; linear extrapolation would slice the wrong sensor range. Tick-anchored slicing is exact regardless of drift because both sensor and GPS rows share the same ThreadX tick counter.

**Float-step frame generation** so the GIF's wall-clock playback length exactly matches the data window. Earlier integer step (e.g. `100/15 = 6` ticks per frame at 15 fps) made the GIF run 1.117× too long, drifting the GIF behind a paired video by ~4 s over a 39 s window. `step_f = sample_hz / fps` (float) + nearest-sample indexing keeps `n_frames = round(samples / step_f)` and the encoded GIF length matches the data length.
