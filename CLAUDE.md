...
**Build versioning — semver `.version`**: the SDDataLogFileX `STM32CubeIDE/` directory keeps a committed `.version` file (currently `0.0.18`) holding a semver-style version string. The Makefile reads it via `$(shell cat .version)` and bakes it into the firmware as `-DFW_VERSION_STRING="X.Y.Z"`. **`make` itself does NOT bump the version** — that was the old behaviour and produced spurious working-tree diffs on every build. Bump explicitly before shipping a release with `make bump` (patch +0.0.1), `make bump-minor` (+0.1.0, resets patch to 0), or `make bump-major` (+1.0.0, resets minor+patch). Three places use the value:

1. **Filename**: `make` emits `build/firmware_v<X.Y.Z>.bin` *alongside* the unversioned `build/firmware.bin` (the latter still required for the SD-update path that looks for an exact filename). Field testers can have multiple .bin files in their Downloads folder and tell them apart at a glance — when sending via WhatsApp use the versioned filename so the conversation log naturally documents which build is in flight.
2. **`FW_INFO_v<X.Y.Z>.TXT`** at the SD root: filename itself carries the version, so the field tester can tell which firmware ran from the SD listing alone — no need to open any file. First line still has the full fingerprint (`fw: v<X.Y.Z> build May  5 2026 08:25:34`). On every boot, the firmware enumerates the SD root and deletes any older `FW_INFO*` files (including the legacy fixed-name `FW_INFO.TXT` from before per-version naming, the older `FW_INFO_v<N>.TXT` integer-counter form, and any prior `FW_INFO_v<X.Y.Z>.TXT` from previously-flashed builds) so the listing only ever shows the current version. Especially important after an SD-update where the bank-swap can silently fail — if the expected `FW_INFO_v<X.Y.Z>.TXT` doesn't appear after copying the new firmware, the swap didn't take.
3. **Error log boot marker**: `--- Boot v<X.Y.Z> May  5 2026 08:25:34 ---` and `fw: v<X.Y.Z> build … flash ~150KB` lines.

`-DFW_VERSION_STRING="X.Y.Z"` is a command-line define which `make` doesn't track as a dependency, so an unchanged `app_filex.c` would otherwise reuse a stale `.o` and a freshly-bumped binary would report the previous version inside. The Makefile has a `$(BUILD_DIR)/app_filex.o: FORCE` rule to always rebuild that single .o (other .o files don't reference `FW_VERSION_STRING`); costs a few seconds per build, cheap insurance for honest version labels. Default fallback for IDE builds (STM32CubeIDE / IAR / Keil) that don't pass `-DFW_VERSION_STRING`: `stbox1_config.h` defines it as `"0.0.0"` so the field tester sees `v0.0.0` and knows it wasn't a CLI build.

**History note**: pre-v0.0.18 the version was a single auto-incrementing integer in `.build_counter`, bumped by every `make` invocation. That produced spurious diffs on every build (`.build_counter` was committed) and made versioning ambiguous between "minor patch" and "release for testing". Switched to semver + manual bump in commit `?` — old `FW_INFO_v<N>.TXT` and `firmware_v<N>.bin` files from earlier builds are still cleaned/ignored by the on-disk cleanup logic.
...
**FW_INFO_v<X.Y.Z>.TXT at SD-card root**: same fingerprint content as the error-log boot marker, but written to a versioned file at the SD root (`FW_INFO_v<X.Y.Z>.TXT` where `<X.Y.Z>` is `FW_VERSION_STRING`) on every boot — no need to grep through `Error_Log_Pump_Tsueri_*.log` or even open any file to confirm which firmware is running; the SD listing tells you. On every boot the firmware walks the SD root and deletes any pre-existing `FW_INFO*` entry (the legacy fixed-name `FW_INFO.TXT`, the older integer-counter `FW_INFO_v<N>.TXT` form, and any prior `FW_INFO_v<X.Y.Z>.TXT` from previously-flashed builds) before writing the new versioned file, so the listing always shows exactly one fingerprint file matching the running binary. Implemented in `WriteFwInfoFile()` in `app_filex.c` via two-pass `fx_directory_first_entry_find` + `fx_directory_next_entry_find` (collect names first, then delete; iterating + deleting in the same pass invalidates the FX directory iterator). Up to 8 stale entries cleaned per boot — generous headroom; the only way to exceed it is to flash 9+ different builds onto a card without ever booting between each, which doesn't happen. Especially important after an SD-update: if the expected `FW_INFO_v<X.Y.Z>.TXT` doesn't appear after copying the new `firmware.bin`, the bank-swap silently failed (known bug — DFU is the workaround). Called from `CheckAndApplyFirmwareUpdate()` right after `fx_media_open` succeeds, so it runs on every boot regardless of whether the user starts a logging session.
