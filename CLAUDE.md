...
**I²C4 throttled to 10 kHz Standard Mode (25.4.2026), then reverted (26.4.2026)**: Peter's 25.4.2026 morning session with the GPIO_PULLUP build still logged `halerr=0x20` — internal pull-ups alone aren't enough to drag SCL/SDA high in time. External pull-ups aren't an option in the field (no parts available in Greece), and Peter pointed out that since only the STC3115 hangs on I²C4 the bus speed can be lowered without affecting anything else. Dropped from `0xA040184A` (~145 kHz Fast Mode) to `0x4F206363` (~10 kHz Standard Mode): `PRESC=0x4F → tPRESC=500 ns, SCLL=SCLH=0x63=99 → SCL period 100 µs`. With internal-pull-up rise-time τ ≈ 40 kΩ × ~50 pF ≈ 2 µs (90 % rise ≈ 4.6 µs), 100 µs / cycle leaves Faktor ~20 of margin. Battery is logged at 1 Hz with a few-byte transfer per sample, so 10 kHz throughput is more than ample. **Reverted to `0xA040184A` on 26.4.2026** while debugging Peter's "2 green blinks then dark" hang — the throttle was never actually field-tested on its own because every flash attempt was landing in the wrong bank (see "Stale-flash-bank recovery" below). Once the bank issue is fully resolved we can re-evaluate whether the 10 kHz value is needed. Change is `bus.c:2189` only.

**Per-sensor diagnostic blinks in InitMemsSensors() (26.4.2026)**: Between `BootStageBlink(2)` and `BootStageBlink(3)`, `main.c` now calls `DiagBlinkRed(n)` before each MEMS sensor init, with a 400 ms gap after each burst, so a "boot hangs at 2 green" symptom can be narrowed down to which specific I²C/SPI sensor transaction blocks. Steps: 1× = entered `InitMemsSensors`, 2× = LIS2MDL (I²C2), 3× = LSM6DSV16X (SPI), 4× = LPS22DF (I²C2), 5× = STTS22H (I²C2), 6× = function exit. Costs ~3 s of boot time but invaluable when serial debug is unavailable. Within hours of shipping this build it gave us the diagnostic signal that **zero red blinks** were emitted — meaning `InitMemsSensors()` was never even called — which combined with multiple identical-hang-after-DFU attempts pointed at a stale-flash-bank problem rather than firmware code (see next note).

**Stale-flash-bank recovery via mass-erase (26.4.2026)**: STM32U585 has dual-bank flash with a `SWAP_BANK` option byte that selects the boot bank. Successful SD-updates toggle this byte, but the bank-swap-bug can leave `SWAP_BANK` in an inconsistent state — DFU then writes new firmware to the inactive bank while the chip continues booting from the active (stale) one. Symptom: DFU reports success but the chip's behaviour never changes after a flash, *and* identical hang behaviour on multiple completely different firmware builds. We confirmed this on Peter's box on 26.4.2026 — three different builds (10 kHz revert, clean 8c0c0878 + FW_INFO.TXT, and the diagnostic build with red blinks before any sensor init) all produced exactly the same "2 green + dark" symptom. The recovery is a `dfu-util` mass-erase before the next flash:
```sh
...
**Resolution: ThreadX byte pool too small for the BLE thread (3.5.2026)**: Peter's no-BLE firmware booted cleanly and wrote everything (FW_INFO, error log, Sens/Gps/Bat CSVs); the BLE-enabled firmware reproduced the dark/empty/silent symptom on the reserve box. Root cause was `TX_APP_MEM_POOL_SIZE = 1024` (1 KB) in `AZURE_RTOS/App/app_azure_rtos_config.h` — when `STBOX1_ENABLE_BLE_SYNC=1`, `BleSync_ThreadX_Init` calls `tx_byte_allocate` for `BLE_SYNC_STACK_SIZE = 4 * 1024` from that 1 KB pool. The allocation returns `TX_NO_MEMORY` (0x10), which propagates: `BleSync_ThreadX_Init` → `App_ThreadX_Init` → `app_azure_rtos.c:tx_application_define`'s error path, which is a bare `while (1) {}`. That loop runs **before `tx_kernel_enter`**, so no thread (including fx_thread) ever gets created. The 3 green `BootStageBlink` calls in `main()` complete because they happen prior to `MX_ThreadX_Init`; everything after that is dead. Bumped the pool to 8 KB (4 KB BLE stack + 4 KB headroom) with a comment in the config header explaining the failure mode so it can't be re-introduced. Same fix idiom that `FX_APP_MEM_POOL_SIZE` already had (it was bumped to 14 KB earlier with a similar history note about NO_MEMORY-after-adding-gps_thread).

The bug went undetected because BLE was never end-to-end-validated on real hardware before shipping — the GUI-side "I see the Scan button" check from earlier was just the GUI launching, not finding a real box. Lesson for any future thread or stack addition: **after bumping any allocation, flash to a real board and confirm the chip reaches `tx_kernel_enter`** (visible signal: green LED goes solid on after the 3 boot blinks). The no-BLE bisect is the cheapest way to confirm the failure is the new code path and not the existing logger.

**Layer 2: empty-FW_INFO + IRQ-storm hypothesis (3.5.2026, post-pool-fix)**: After the pool fix, Peter's reserve box now creates `FW_INFO.TXT` on the SD — but it's **0 bytes**, and no other files appear (no Sens/Gps/Bat, no `Error_Log_Pump_Tsueri_*.log`, no `STBoxSync` BLE advertisement, green LED never goes solid). FAT directory entries are atomic, so the create operation completing while the subsequent `fx_file_write` doesn't means fx_thread reached `WriteFwInfoFile` then stalled mid-write. The error log is created from inside `case COMMAND_START_LOG` (sent automatically by `read_thread` on boot), so its absence means fx_thread never returned to its main loop after `CheckAndApplyFirmwareUpdate`.

The leading hypothesis: `init_ble_int_for_blue_nrglp` in `Core/Src/ble_implementation.c:56` configures `HCI_TL_SPI_EXTI_IRQ_N` at NVIC priority 0 (highest possible — `HAL_NVIC_SetPriority(HCI_TL_SPI_EXTI_IRQ_N, 0, 0)`). On a reserve box where the WB07_06 chip isn't responding cleanly the IRQ line probably floats or chatters, and at NVIC priority 0 a continuous EXTI11 storm preempts SDMMC interrupts and even the ThreadX SysTick. fx_thread is preempted mid-write and never resumes. Atomic FAT create completed (FW_INFO entry), `fx_file_write` did not (FW_INFO empty). No SD I/O completes after that point, no LED state change, no BLE advertise.

Diagnostic to confirm: add a 10 s `tx_thread_sleep(1000)` at the very top of `ble_sync_thread_entry`. fx_thread gets uncontested time to write FW_INFO, process the auto-`COMMAND_START_LOG`, and create Sens/Gps/Bat + error log. After the sleep, BLE init runs — and if the hypothesis holds, the chip then goes dark (LED off, no further writes), but the SD will have a complete pre-BLE log. **This delay is a probe, not a fix** — kept in the working tree only while the diagnostic firmware is in Peter's hands; reverted before commit. The actual fix once confirmed is two parts: (a) drop the EXTI priority below ThreadX's kernel mask (typically 5+ on STM32U5), (b) add a chip-presence check (e.g. read a known register) before enabling the IRQ at all, so a non-responding chip is detected and the BLE thread bails out instead of arming the IRQ.

**Confirmed + fixed: EXTI11 priority-0 storm + ISR endless-loop (4.5.2026)**: Peter's 3.5.2026 19:38 build produced the smoking gun — both `Sens000.csv` and `Bat000.csv` stop at exactly tick 936 (~9.4 s after boot), `Bat001.csv` from a separate boot stops at tick 946 (~9.5 s), `STBoxSync` never advertises, green LED freezes at whatever the last `BSP_LED_Toggle(LED_GREEN)` left it. Both files cut off at the *same* tick on each run = fx_thread preempted mid-batch. `bluetooth_init()` takes ~8–10 s for the SPI/HCI handshake + GAP setup, then `init_ble_int_for_blue_nrglp()` arms EXTI11 at NVIC priority 0. On boxes where the BlueNRG-LP IRQ line chatters or sticks high, the ISR loops on `while (is_data_available())` forever — at priority 0 it preempts SDMMC (14), TIM6/SysTick, and never returns to PendSV, so no thread ever runs again. The randomness Peter sees ("50:50 LED stays solid or goes off after boot") is just *which* state the green LED was last toggled to before fx_thread froze, not anything about boot success. Three-part fix in commit `<this commit>`:

1. **Bound the ISR loop** in `Core/Src/hci_tl_interface.c::hci_tl_lowlevel_isr` to a hard max of 16 events per IRQ. Even at priority 0, the handler now returns within bounded time and the next pending edge re-fires it through the NVIC instead of spinning inside one entry.
2. **Lower NVIC priority** of `HCI_TL_SPI_EXTI_IRQ_N` from 0 to 14 in `Core/Src/ble_implementation.c::init_ble_int_for_blue_nrglp`. Same level as SDMMC1 (`BSP_SD_IT_PRIORITY = 14`) so an HCI event never preempts an in-flight SD write, and well below UART4-GPS at 6 so the GPS line keeps assembling. SysTick is at 15 (lowest) — already below us. We deliberately did *not* go to priority 15 because that would let the BLE chip's normal HCI traffic be preempted by a stuck SDMMC handler, which can deadlock if SDMMC is itself waiting on a BLE-clock-domain resource.
3. **Chip-presence gate** in `Core/Src/ble_sync.c`: `bluetooth_init()` now returns the `init_ble_manager()` status (was `void`). On non-zero rc, the BLE thread writes `ble: init FAIL rc=N - thread bailing` to the error log and parks itself in a `tx_thread_sleep(1000)` loop *without* ever calling `init_ble_int_for_blue_nrglp`. fx_thread keeps logging untouched. On success, the thread writes `ble: init ok - arming EXTI11 at NVIC prio 14` first, so post-session we can tell from the log whether BLE was up or skipped. Header signature changed: `extern uint8_t bluetooth_init(void);` in `Core/inc/ble_implementation.h`.

What to look for in the next field-test log:
- `ble: init ok - arming EXTI11 at NVIC prio 14` followed by working `STBoxSync` advertise + GUI-visible scan = full fix landed.
- `ble: init FAIL rc=N - thread bailing` + green LED toggling at 1 Hz the whole session + complete `Sens/Gps/Bat` CSVs = chip is bad on this box but logger no longer hostage to it.
- LED freeze + truncated CSVs + no `ble: init` line in the error log = somehow we still got into the IRQ before the gate; investigate further (e.g., is `init_ble_manager` returning success even when the chip is dead?).
...
**GPS-tick anchored slicing — both ends (26.4.2026)**: the at-window is anchored by finding the GPS row whose UTC is closest to the requested wall-clock time and using its tick directly, rather than linearly extrapolating from the first GPS sample. ThreadX runs on HSI (internal RC, ±1 % accuracy) and drifts ~7 s over a 21-min session; linear extrapolation would slice the wrong sensor range. Tick-anchored slicing is exact regardless of drift because both sensor and GPS rows share the same ThreadX tick counter. **Both** ends of the window now use this — earlier the start was GPS-anchored but the end was computed as `anchor_tick + dur*100`, which assumes 100 ticks = 1 wall-clock-second. For Ayano's 25.4.2026 41-sec at-window that ~1 % HSI drift compounded with the GIF-rate rounding (below) into ~5 s of late GPS-track-marker position vs. the camera video at the end. Now the end-anchor finds the GPS row at `anchor_utc + dur` UTC and uses its tick, so the data window equals exactly `dur` real seconds even on long sessions.

**Float-step frame generation + GIF-spec delay rounding (26.4.2026)** so the GIF's wall-clock playback length exactly matches the data window. Earlier integer step (e.g. `100/15 = 6` ticks per frame at 15 fps) made the GIF run 1.117× too long, drifting the GIF behind a paired video by ~4 s over a 39 s window. Float `step_f = sample_hz / fps` + nearest-sample indexing fixed that *for the data side*. But there's a second pitfall: GIF spec encodes per-frame delay in 1/100-sec units (10 ms resolution), so `1000/15 = 66.67 ms` gets rounded by the gif crate to `70 ms` → real playback rate 14.286 fps, not 15. If `step_f` is still computed from the 15-fps target the GIF plays ~5 % slower than the data step. Now we pre-round `delay_ms` to a 10-ms multiple, derive `effective_fps = 1000 / delay_ms`, and use that for `step_f` so the encoded GIF rate matches the data step. Validated on Ayano 25.4.2026 at101609: 40.9-sec at-window → 584-frame GIF at 14.286 fps = 40.88 sec playback (was 41.02 sec for 60.6 sec of data before the fix).
