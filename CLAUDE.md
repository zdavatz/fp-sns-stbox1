...
3. **ThreadX byte-pool size** — `TX_APP_MEM_POOL_SIZE` in `AZURE_RTOS/App/app_azure_rtos_config.h` is 16 KB (was 1 KB → 8 KB → 16 KB). `BleSync_ThreadX_Init` allocates 4 KB for the BLE thread stack from this pool. At 1 KB the allocation returned `TX_NO_MEMORY` and `tx_application_define`'s error path ran *before* `tx_kernel_enter`, so no thread (including fx_thread) was ever created. 8 KB worked on Zeno's box but Peter's rebuilt box still hit the same symptom — likely `tx_byte_pool` internal headers + free-block padding + 8-byte alignment of the 4 KB stack pushing the effective requirement just over 8 KB on his build. 16 KB has comfortable margin; the cost is 8 KB of static BSS (rounding error on 768 KB SRAM). The 3 green `BootStageBlink` calls in `main()` still complete on a too-small pool because they happen prior to `MX_ThreadX_Init`. Same idiom as `FX_APP_MEM_POOL_SIZE` (14 KB after gps_thread was added).
4. **LED morse on `tx_application_define` halts** — the four error paths (tx pool create, App_ThreadX_Init, fx pool create, MX_FileX_Init) used to drop into a silent `while(1){}`, indistinguishable from any other pre-RTOS hang. Now they call `halt_red_morse(N)` in `app_azure_rtos.c`: red LED blinks N times, pauses 1.2 s, repeats. Patterns: 2 = tx pool failed, 4 = App_ThreadX_Init (BLE thread alloc/create) failed, 6 = fx pool failed, 8 = MX_FileX_Init failed. So a "3 green blinks then dark" is now self-diagnosing.

Lesson: **after bumping any allocation, flash to a real board and confirm the chip reaches `tx_kernel_enter`** — visible signal: green LED goes solid on after the 3 boot blinks (or red-LED morse if a pool/init step failed; count the blinks against the table above). The cheapest way to confirm a BLE-init regression vs. an existing-logger regression is to build with `STBOX1_ENABLE_BLE_SYNC 0` (drops ~28 KB flash + 4.5 KB BSS, byte-identical logger) and bisect.

**VddIO2 power-island enable**: `HAL_PWREx_EnableVddIO2()` runs in `Core/Src/stm32u5xx_hal_msp.c::HAL_MspInit`. STM32U585 gates the GPIO bank fed by VddIO2 (PG[15:2] on the Rev_C package) behind a separate IO supply rail; without the explicit enable, reads/writes on those pins silently fail. ST's reference BLE firmware (BLEDualProgram) calls it in both `HAL_MspInit` and `main`; the SDDataLogFileX `HAL_MspInit` was missing it for several firmware revisions, which surfaced as `bluetooth_init()` returning non-zero on a chip that worked fine under the stock ST firmware — the chip-presence gate above (correctly) parked the BLE thread, masking what was actually a power-island problem rather than a dead BlueNRG-LP. Cost is ~16 B flash and one bit in `PWR->SVMCR`; cheap to keep enabled even if no BLE-relevant signal turns out to live on a VddIO2 pin. **If the error log shows `ble: init FAIL` on a chip that worked under unmodified ST firmware, this is the first thing to verify is still in `HAL_MspInit`** — easy to lose during a CubeMX regenerate.
...
The probe is **necessary but not sufficient on the 3.3V-modded box** — even on builds where BLE is fully disabled (`STBOX1_ENABLE_BLE_SYNC 0`), the same hardware showed non-deterministic SDMMC hangs at random `fx_file_*` operations within the first 50 ticks (~500 ms) of fx_thread (different boots stopped at `fx_file_create`, `fx_file_open`, or `fx_file_seek`, with no pattern). This ruled BLE out as the sole cause and pointed at SDMMC signal/power-integrity on the modded supply. **Mitigation in `ea4bd38a` (Drivers/BSP/SensorTileBoxPro/SensorTileBoxPro_sd.c): SDMMC kernel clock lowered from `SDMMC_HSPEED_CLK_DIV` (~25 MHz) to `SDMMC_NSPEED_CLK_DIV` (~12.5 MHz)** to give the 3.3V rail more setup/hold margin. Throughput at NSPEED (~6 MB/s ceiling) is still well above the logger's ~5 KB/s budget; cost is negligible. Workaround that stays useful regardless: `STBOX1_ENABLE_BLE_SYNC 0` build improves the ratio of successful boots compared with BLE-on, since the BLE thread + interrupt path adds bus contention to an already-tight margin.

**Field test results (issue #12)**: NSPEED alone wasn't enough on Peter's box. v10 (with NSPEED) reached `fx: COMMAND_START_LOG enter, before fx_file_create Sens` at tick 41 then hung — pinning the failure to `fx_file_create` for the freshly-named `Sens000.csv`. We hypothesised FAT corruption from prior 3.3V-bad-write sessions and asked Peter to reformat the SD card (FAT32, MBR — `diskutil eraseDisk MS-DOS PUMPFOIL MBRFormat /dev/diskN` on macOS); v11 on the freshly-formatted card regressed *earlier* — only the 4 boot/fw/reset/ble lines from `ErrorLog_Open` made it to disk, and the very next `ErrorLog_Write` never landed. **This killed the FAT-corruption hypothesis**: a clean card behaves the same way. Reset reason `POR (CSR=0x0C004400)` confirmed clean cold boot, no software fault — fx_thread was still alive (green LED solid on for 30+ s) but its writes weren't reaching the card.

The v11→v13 bisect localized the problem to `fx_media_flush` itself: `ErrorLog_Open` writes 4 lines and flushes *once* at the end, and that flush succeeds; the *next* `fx_media_flush` (called by the per-line `ErrorLog_Write` path) hangs. Best fit: SD-card post-write busy state that the SDMMC driver doesn't poll correctly under marginal 3.3V signal integrity — the card needs ~hundreds of ms of internal housekeeping after a multi-sector flush, the next CMD13 read-back of card-ready never gets a valid response, and the driver spins waiting forever. The cure that stays in firmware scope (without re-architecting the SDMMC driver's busy-wait) is to **stop calling `fx_media_flush` after every line**: see "Per-step ErrorLog markers" below.
...
**Batched-flush write pattern (v13+)**: `ErrorLog_Write` no longer calls `fx_media_flush` after every line — that pattern was found to hang on Peter's 3.3V-modded box right after `ErrorLog_Open`'s big-batch flush succeeded (see issue #12 paragraph above). Instead, writes accumulate in the FileX cache and are committed in batches at strategic checkpoints — matching the working many-writes-one-flush pattern that `ErrorLog_Open` itself uses. There are two ways to commit:
1. **Implicit checkpoints**: every per-file flush already in the START_LOG body (after the Sens header, after the GPS header, after the Bat header) flushes the *whole* media cache including any buffered ErrorLog lines.
2. **Explicit `ErrorLog_Flush()`**: a separate function (declared in `Core/inc/main.h`, defined alongside `ErrorLog_Write`) that just calls `fx_media_flush(&sdio_disk)` and returns the FX status. Use sparingly — each call carries the same potential-hang risk that motivated removing the per-line flush in the first place. Currently used right after the manifest write and after each major fx-step status marker so we always have at least the most recent operation's outcome on disk.

**Forward-looking manifest pattern**: right after `ErrorLog_Open` returns, `COMMAND_START_LOG` writes a batch of seven "fx: about to call <op>" lines covering every risky operation up through the post-header flush, then calls `ErrorLog_Flush` once to commit them. Idea: even if a *later* flush hangs, all seven manifest markers are already on disk, so the post-session log tells us exactly which step the firmware was about to attempt — no need to infer from incomplete `before X` / `X returned` pairing. The trailing "X returned status=0xN" markers fill in as later flushes succeed; the manifest is the floor.

**`DiagBlinkRed` is now public** (declared in `Core/inc/main.h`) so other translation units can use the same red-LED-pulse helper as `InitMemsSensors`. Currently no callers outside `main.c` — kept exported so a future `app_filex.c` diagnostic can fall back to LED counting when SD writes are dead. The on-disk markers above are the primary diagnostic; `DiagBlinkRed` is the safety net if a future regression takes the file path down completely.

**BlueNRG-LP OTP / WLC flashing — root cause behind issue #12**: the SensorTile.box PRO has two MCUs on board: the **STM32U585** (runs SDDataLogFileX, sensors, SD, GPS) and the **BlueNRG-LP** (separate ARM core dedicated to BLE, talked to via SPI1 + EXTI11 IRQ). The BlueNRG-LP doesn't ship from the factory with a usable BLE stack — it has an `iload` ROM bootloader and a one-time-programmable patch area that has to be **OTP-burned with a stack image (`dtm.bin`, 200 KB)** before the chip can answer HCI commands sensibly. Without OTP-programmed patches the chip is in an undefined state: SPI bytes go in, the chip's behaviour is non-deterministic — random IRQ pulses, current spikes, and on the 3.3V-modded reservebox enough power-rail noise to corrupt SDMMC writes happening on the same supply. This is what we mis-diagnosed for ~30 firmware iterations (v6 through v37) as a concurrency / mutex / timing problem with the SDMMC peripheral.

The diagnostic chain that finally exposed it: the user supplied two binaries from ST — `SensorTile.boxPRO.bin` (ST original firmware) and `dtm.bin`. Running `strings` on `SensorTile.boxPRO.bin` showed `[WLC] OTP Programming started`, `[WLC] Loading RAM FW!!`, `[WLC] Iload Reg value: 0x%02X`, `[WLC] Cut Id`, etc. — a complete WLC (Wireless LE Controller) flasher the ST firmware runs on every boot to either verify the OTP is up-to-date or program it from `dtm.bin` on the SD. Our SDDataLogFileX firmware **does not** include that flasher; we inherited the BLE init pattern from `BLEDualProgram` which assumes the chip is already programmed. So a box where the OTP wasn't burned (or got corrupted) fails everything BLE-related the moment our `bluetooth_init` runs.

Recovery procedure (one-shot per box, see top-level `README.md` "First-time BlueNRG-LP OTP setup"): put `dtm.bin` on SD, DFU-flash `SensorTile.boxPRO.bin`, boot, the ST firmware burns the OTP. Permanent — the OTP fuses survive all subsequent flashes. Then DFU-flash our SDDataLogFileX and BLE works.

The proper fix in our firmware is to integrate ST's WLC SPI flasher (`DTM_Updater_SPI.c` from the `STSW-BNRGLP-DK` package on st.com — license-clickthrough zip download, not on GitHub) so our firmware self-heals on boot if it sees an unprogrammed BlueNRG-LP. **TODO** — meanwhile the manual setup is documented for field testers. Lesson learned for future ST-silicon debugging: when something looks like a hardware/timing issue spanning weeks of iteration, dump strings from the official ST reference binary first; the WLC-init machinery being missing in our firmware was visible in 60 seconds of `strings | grep` once we thought to look.

**WLC port progress (untested)**: Rather than wait for the STSW-BNRGLP-DK click-through, the seven WLC functions (main + RAM-FW loader + OTP writer + chip-info reader + 2 chunked-I/O helpers + cleanup) were reverse-engineered from `SensorTile.boxPRO.bin` via Ghidra headless and ported to `Core/Src/wlc.c` (~3 KB text + ~12 KB BSS). Public API: `WLC_CheckAndProgram(const uint8_t *dtm, size_t dtm_size)` returning a `wlc_status_t` enum (NOT_REQUIRED / PROGRAMMED / PROBE_FAILED / BAD_DTM / PROG_FAILED / VERIFY_FAILED). Reuses the existing SPI1 infrastructure (`Core/Src/ble_spi.c::BleSpi_SendRecv` + `Core/Src/hci_tl_interface.c::hci_tl_spi_reset`); manually toggles CS on PA2 around each WLC packet (no HCI framing). Wire format reconstructed: WRITE packets are `[0xFA, 4-byte BE addr, data...]`; OTP register writes use `[2-byte BE reg, data...]` with 100 ms HAL_Delay after; reads are opcode-byte (0x00=chip-info / 0x06=version / 0x0E=op-mode) followed by N receive bytes, all CS-held-low. OTP-write completion is polled on PE3 (decoded from `DAT_08069304 = 0x42021000` = GPIOE base + bit 3 in IDR). Build-gated on `STBOX1_ENABLE_WLC` in `stbox1_config.h` (default 0); flag-off compiles to a stub returning `WLC_STATUS_DISABLED` so logger builds remain byte-identical. Flag-on currently has NO call site — the boot-path wiring (read `dtm.bin` from SD, call before BLE init) is intentionally deferred until real-hardware validation, so flipping the flag without writing a call site only adds dead code that the linker GC strips. Bug fixed during compile: the trailer offset in the both-cfg-and-patch staging case is `cfg_size + patch_size = 0xFA0` (4000 decimal in the decompiler output), NOT `0x3FA0` — that hex constant is a separate "OTP available space ceiling" check elsewhere in `FUN_08068f50`.

**Plot twist — WLC code in ST's binary is unreachable**: a brute-force xref scan (`/Users/zdavatz/ghidra_scripts/FindXrefs.java`) over the entire 11 MB `SensorTile.boxPRO.bin` found **zero** references to `FUN_08068f50` (the WLC main entry point) or its thumb-mode +1 alias. No direct callers, no function-pointer storage, no jump-table entry. The strings (`[WLC] OTP Programming started` etc.) are only ever referenced from inside `FUN_08068f50` itself; the function logs them but no one calls the function. So the WLC subsystem is **legacy/dead code** carried in the firmware image — built but never invoked. ST's actual production BLE bringup is `FUN_08031468` (580 B, found via the `BlueNRG-LP HWver %d FWver %d` log string), and it does NOT call WLC. Implication: the manual OTP-setup procedure documented in README.md (DFU + dtm.bin on SD + ST original FW) **does not actually program OTP via this firmware**. Either the chip ships pre-programmed from the factory, or some separate factory-flasher tool handles OTP, or there's an alternate code path (early Reset_Handler? boot strap?) that we haven't found yet. Practical consequence: our `Core/Src/wlc.c` port is a theoretical exercise; there's no proof the wire protocol it implements would actually work against a real unprogrammed BlueNRG-LP, because we don't know the production flasher's protocol either. Keep `STBOX1_ENABLE_WLC = 0` until someone produces a definite OTP-programming reference.

**The actual production-init flow** (`FUN_08031468` decompiled, vs. our `init_ble_manager`):
1. `FUN_08043794` — HCI transport init (callbacks + 100 queue slots in a circular buffer at `DAT_08043a0c`)
2. `FUN_0804c3bc` — event-handler registration, callback ID 0x16
3. `FUN_0804cff6` — sends **HCI Reset** (opcode `0x0C03`, OGF=3 OCF=3) and waits for Command Complete via `FUN_08043804` (the HCI command sender — 1 sec inter-event timeout)
4. **`FUN_0802d5d8(2000)` — 2 second post-reset delay**
5. `FUN_080317b0` — sends **HCI Read Local Version** (opcode `0x1001`, OGF=4 OCF=1), unpacks HW + FW version
6. `FUN_0804d1c8(0x80, ...)` — sends **ACI Read Config Data** (opcode `0xFC0D` with offset byte `0x80`), reads 300-byte response
7. checks `(resp[0x24] & 0xC0) == 0xC0` — OTP-OK / Public-Address-set flags
8. standard GATT init (`FUN_08047880`), GAP init (`FUN_0804caae`), I/O capability config, advertising-params + Tx-power setup

HCI command framing on the wire (`FUN_080436c6`): `[0x01 cmd_type][opcode_LE16][len][params]`, ACL data variant `[0x81 type][handle_LE16][length_LE16][data]`. Standard BlueNRG SPI-V2. Our `hci_tl_interface.c::hci_tl_spi_send` does the same wire format. **The only step our middleware doesn't have is step 4 — the 2-second post-HCI-Reset settle.** Cause: BlueNRG-LP needs that gap after a reset to finish RF subsystem bring-up before answering follow-up commands. Without it, our middleware fires HCI Read Local Version into a transient-state chip that doesn't respond, and the BLE thread (priority 11) gets stuck inside `init_ble_manager` waiting for a response that never comes — fx_thread (priority 12) is then starved, SD never opens, LEDs stay dark after the boot beep. That signature matches Peter's v41 reservebox failure exactly.

**Mitigation in v48** (`Core/Src/ble_sync.c::ble_sync_thread_entry`): `HAL_Delay(2000)` between the chip-alive probe success and `bluetooth_init()`. Mirrors ST's production pattern. Costs 2 s of boot latency on every successful BLE bring-up; cheap insurance for a regression fix when the alternative is "logger doesn't work at all on the modded box". If real-hardware testing shows the delay is overkill, drop to 1000 ms or experiment with shorter values.

The earlier `g_ble_probe_status` codes (0=probe failed, 1=advertising, 2=init failed, 0xFF=pending) are extended from v39 with intermediate `0xFx` markers (0xF0=thread entered, 0xF1=in `ble_chip_alive_probe`, 0xF2=in `bluetooth_init`, 0xF3=transient post-init, 0xF4=in `init_ble_int_for_blue_nrglp`) so the periodic logger in `app_filex.c` `COMMAND_SAVE_SENSORS` can pinpoint exactly which BLE init step hangs when one does. Implemented in `Core/Src/ble_sync.c::ble_sync_thread_entry` and the matching switch in `app_filex.c`.
...
2. **`FW_INFO_v<N>.TXT`** at the SD root: filename itself carries the version, so the field tester can tell which firmware ran from the SD listing alone — no need to open any file. First line still has the full fingerprint (`fw: v<N> build May  5 2026 08:25:34`). On every boot, the firmware enumerates the SD root and deletes any older `FW_INFO*` files (including the legacy fixed-name `FW_INFO.TXT` from before per-version naming and any prior `FW_INFO_v*.TXT` from previously-flashed builds) so the listing only ever shows the current version. Especially important after an SD-update where the bank-swap can silently fail — if `FW_INFO_v<N+1>.TXT` doesn't appear after copying the new firmware, the swap didn't take.
...
**Issue #12 root cause CONFIRMED (2026-05-08): the BLE manager DEPENDENCY CHAIN at link-time, not anything at runtime.** All previous theories (3.3V power integrity, OTP/WLC, SDMMC busy-wait timing, EXTI11 priority, two-flushes-too-close) were either red herrings or partial truths that didn't explain the unmodded box also reproducing the bug.

The local-DFU debugging pipeline on Zeno's box (USB-C + `dfu-util mass-erase:force:leave` — no WhatsApp roundtrip with field tester needed) made the bisect tractable. Each iteration is ~30 seconds.

The 5-step bisect that nailed it:
1. **Pre-BLE firmware (commit `2268ae15`, 2026-04-28):** rebuilt fresh, flashed, `Sens000.csv` 413 KB after 60 s. Logger 100% functional.
2. **First BLE commit (`c6d3fa5b`, 2026-05-02) with `STBOX1_ENABLE_BLE_SYNC=0`:** linker DCEs all BLE manager code because nothing references `init_ble_manager`. Binary back down to 117 KB (vs 149 KB BLE-on). `Sens001.csv` 633 KB. **BLE wiring at the source-tree level isn't the bug.**
3. **`STBOX1_ENABLE_BLE_SYNC=1`, BLE thread spawned, `bluetooth_init()` commented out:** `init_ble_manager` still unreferenced → DCE'd → 122 KB binary. `Sens002.csv` 594 KB. **The BLE thread itself isn't the bug.**
4. **`STBOX1_ENABLE_BLE_SYNC=1`, `bluetooth_init()` called after a `tx_thread_sleep(500)` (5 sec):** linker now keeps `init_ble_manager` and the entire BLE manager dependency chain reachable → binary jumps to 149 KB. Logger stops at tick ~200 (~2 sec) — `Sens003.csv` only 26 KB. **Even though `bluetooth_init` doesn't actually run for the first 5 seconds, the linker output already breaks the logger.**
5. **`STBOX1_ENABLE_BLE_SYNC=1`, `bluetooth_init()` body stubbed to empty (no `init_ble_manager` reference inside) but still called from BLE thread:** linker DCEs `init_ble_manager` again → 122 KB. `Sens004.csv` ~600 KB. **Confirms the bug is the LINKED `init_ble_manager` dependency chain, not the `bluetooth_init` symbol or the BLE thread runtime call sequence.**

So the smoking gun: adding `init_ble_manager`'s ~30 KB of code + ~20 KB of BSS shifts the memory layout of the broken binary in a way that breaks `fx_media_flush` after the second flush. The runtime call-chain involving SPI / HCI / EXTI11 isn't relevant; the bug fires before any of that runs. The earlier "two-flush hang" hypothesis was correct in symptom but wrong in cause — the second flush hang is a function of *layout*, not of timing.

Likely mechanisms (next-day investigation):
- `media_memory[]` (FileX media buffer) ends up at an alignment that interacts badly with SDMMC1 IDMA after the BSS grows by 20 KB.
- One of the thread stacks (`fx_byte_pool_buffer`, `tx_byte_pool_buffer`) bumps against a static buffer (e.g., `Audio_OUT_Buff` at 32 KB, `hci_read_packet_buffer` at 2.7 KB) so an overflow stomps SDMMC state.
- Some BLE-manager static buffer aliases a memory range that the SDMMC driver assumes is private.
- Cortex-M33 ICACHE behavior changes once `.text` exceeds a power-of-two boundary, surfacing a previously-invisible cache coherency issue with IDMA writes.

Workaround that ships TODAY for field testers: `STBOX1_ENABLE_BLE_SYNC = 0`. Logger 100% functional, no BLE FileSync. Build size is 117 KB and matches the pre-BLE behaviour byte-for-byte after DCE.

Diagnostic next steps:
1. **Linker map diff:** `arm-none-eabi-objdump --section-headers` on both the working (117 KB) and broken (149 KB) ELFs, compare the addresses of `media_memory`, `Audio_OUT_Buff`, `tx_byte_pool_buffer`, `fx_byte_pool_buffer`, `sdio_disk`. Whichever symbol's address has shifted across a meaningful boundary (e.g., out of a region with cache attributes) is the suspect.
2. **Bisect through BLE-manager source files:** instead of all-or-nothing on `bluetooth_init`, individually reference subsets — only `init_ble_manager_ble_stack`, only `init_ble_manager_services`, only `aci_gap_set_advertising_*`, etc. Find the single function whose linkage pulls the binary across the broken threshold.
3. **Pin SDMMC media buffer in the linker script:** if the issue is buffer alignment, force `media_memory` into a fixed `0x20000000` region so it never moves regardless of BSS growth.
4. **Try building with `--print-memory-usage` and `--verbose`** to see exactly what the linker is doing with the BSS layout, including any padding insertions.
...
**Audible boot signals via piezo on PE13** (TIM1_CH3 PWM, driver in `Core/Src/buzzer.c`): two cues let a field tester confirm the box is healthy without watching the LEDs. (1) `Buzzer_BootDone()` — two ascending tones (1500 Hz / 3000 Hz, 90 ms each, ~240 ms total) fired from `main()` right before `MX_ThreadX_Init`. If you hear the two-tone followed by green-LED-solid, every pre-RTOS init succeeded and `fx_thread` is running — much louder/clearer signal than the 3-blink sequence in sunlight. (2) `Buzzer_FixAcquired()` — three quick 2500 Hz chirps (60 ms each, ~270 ms total) on the first valid GPS fix from `gps_thread`, fired once per boot via a static latch. Distinct enough from the boot two-tone that you can tell apart "box just booted" from "GPS just locked". `HAL_Delay()` inside `Buzzer_Beep` is safe in both call sites — `Buzzer_BootDone` runs pre-ThreadX, and `Buzzer_FixAcquired` runs from `gps_thread` where blocking 270 ms once per boot is harmless (UART IRQ keeps filling the ring buffer).
...
5. UBX-CFG-VALSET → set CFG-NAVSPG-DYNMODEL = 5 (Sea). The Portable default suppresses Doppler velocity reporting on water (ground speed clamps near 0 even while position is clearly advancing); Sea trusts the velocity and reports realistic km/h. **Layers must be 0x01 (RAM only)**, not 0x07 (RAM | BBR | Flash). Some MAX-M10S firmware variants reject the multi-layer form atomically and the module logs `dyn=NAK(Sea)` in the GPS init log, leaving the dynamic model at the factory default. Persistence is preserved because the next step (UBX-CFG-CFG) writes RAM → BBR/Flash/EEPROM in one shot.
6. UBX-CFG-CFG → save all sections (BBR + Flash + EEPROM) so the config survives power cycles.
...
**FW_INFO_v<N>.TXT at SD-card root**: same fingerprint content as the error-log boot marker, but written to a versioned file at the SD root (`FW_INFO_v<N>.TXT` where `<N>` is `FW_BUILD_NUM`) on every boot — no need to grep through `Error_Log_Pump_Tsueri_*.log` or even open any file to confirm which firmware is running; the SD listing tells you. On every boot the firmware walks the SD root and deletes any pre-existing `FW_INFO*` entry (the legacy fixed-name `FW_INFO.TXT` and any older `FW_INFO_v*.TXT` from previously-flashed builds) before writing the new versioned file, so the listing always shows exactly one fingerprint file matching the running binary. Implemented in `WriteFwInfoFile()` in `app_filex.c` via two-pass `fx_directory_first_entry_find` + `fx_directory_next_entry_find` (collect names first, then delete; iterating + deleting in the same pass invalidates the FX directory iterator). Up to 8 stale entries cleaned per boot — generous headroom; the only way to exceed it is to flash 9+ different builds onto a card without ever booting between each, which doesn't happen. Especially important after an SD-update: if `FW_INFO_v<N+1>.TXT` doesn't appear after copying the new `firmware.bin`, the bank-swap silently failed (known bug — DFU is the workaround). Called from `CheckAndApplyFirmwareUpdate()` right after `fx_media_open` succeeds, so it runs on every boot regardless of whether the user starts a logging session.
