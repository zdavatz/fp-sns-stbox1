...
**Two-stage BLE chip-alive probe (5.5.2026)**: an initial single-stage probe (just check `hci_tl_spi_send` returned 0 = SPI ACK) was too lax — Peter's 3.3V-modded reservebox has a half-dead BlueNRG-LP that ACK's SPI bytes but then hangs the kernel inside `init_ble_manager`'s GATT/GAP setup, which starves `fx_thread` mid `fx_file_create` on Sens000.csv. Tightened to: (1) `hci_tl_spi_send(HCI_Reset)` must return 0 (chip raised IRQ in <15 ms when we asked to send), AND (2) chip must raise IRQ again within 500 ms with a Command Complete response. If either stage fails, `bluetooth_init` is never called and the BLE thread parks; logger keeps writing. Implementation in `Core/Src/ble_sync.c::ble_chip_alive_probe()`. Bounded worst case: 5 ms RST low + 150 ms boot wait + 15 ms TX timeout + 500 ms RX timeout = ~670 ms. The probe outcome is recorded in a global `g_ble_probe_status` (0=dead, 1=alive+init OK, 2=alive but init failed, 0xFF=still pending) which `ErrorLog_Open()` snapshots into a single line on the SD so post-session you can tell from the log whether BLE was up or skipped.
...
**Per-step ErrorLog markers in COMMAND_START_LOG (5.5.2026)**: `app_filex.c`'s `COMMAND_START_LOG` case writes `ErrorLog_Write` markers between every `fx_file_*` call so non-deterministic hangs can be pinned exactly. Markers: `fx: COMMAND_START_LOG enter, before fx_file_create Sens` → `fx: fx_file_create Sens returned` → `fx: before fx_file_open Sens` → `fx: fx_file_open Sens returned status=0xN` → `fx: before fx_file_seek Sens` → `fx: fx_file_seek Sens returned status=0xN` → `fx: before fx_file_write Sens header` → `fx: fx_file_write Sens header returned status=0xN` → `fx: before fx_media_flush after Sens header` → `start: sens header written`. Status code is sprintf'd in so a `0x9` (FX_INVALID_PATH) or similar non-zero result on a non-hanging failure gets captured before `Error_Handler` closes the log. Each marker pair brackets one `fx_file_*` call; the LAST marker on disk pinpoints which call hung. The cost is ~10 extra `ErrorLog_Write` invocations per session start (each is ~1 SDMMC sector write + flush), ~50 ms of extra start-up latency.

**Build versioning — `FW_BUILD_NUM` counter (5.5.2026)**: each `make` invocation in the SDDataLogFileX `STM32CubeIDE/` directory bumps a counter in `.build_counter` (**committed** so build numbers are globally unique across the repo, not per-developer; means a normal `make` produces a working-tree change to that file that should be committed alongside any source changes that produced the binary you're shipping) and bakes it into the firmware as `-DFW_BUILD_NUM=N` via a `$(shell …)` expression. Three places use the value:
...
`-DFW_BUILD_NUM=N` is a command-line define which `make` doesn't track as a dependency — so an unchanged `app_filex.c` would otherwise reuse a stale `.o` and v<N+1>.bin would actually report `v<N>` inside (real bug encountered during the 5.5.2026 reservebox debugging session, where a v2 binary contained v1's number). Fix: `$(BUILD_DIR)/app_filex.o: FORCE` rule in the Makefile forces this single .o to always rebuild (other .o files don't need it because they don't reference `FW_BUILD_NUM`). Costs a few seconds per build; cheap insurance for honest version labels. Default fallback for IDE builds (STM32CubeIDE / IAR / Keil) that don't pass `-DFW_BUILD_NUM`: `stbox1_config.h` defines it as `0` so the field tester sees `v0` and knows it wasn't a CLI build.
