...
**Build versioning — `FW_BUILD_NUM` counter (5.5.2026)**: each `make` invocation in the SDDataLogFileX `STM32CubeIDE/` directory bumps a counter in `.build_counter` (gitignored) and bakes it into the firmware as `-DFW_BUILD_NUM=N` via a `$(shell …)` expression. Three places use the value:
