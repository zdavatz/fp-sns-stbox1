...
**In-app updater (v0.1.9+).** Ported directly from `~/.software/old2new/create_shorts_gui` (`update.rs` + `installer.rs`). On startup `spawn_update_check()` hits `GET https://api.github.com/repos/zdavatz/fp-sns-stbox1/releases?per_page=30` (15 s timeout, `rustls-tls` so no OpenSSL/Schannel dep), filters non-prereleases with tag prefix `v`, picks the highest version strictly greater than `env!("CARGO_PKG_VERSION")`, and pulls out the asset whose name ends with `-macos-aarch64.dmg`. A blue banner at the top of the central panel surfaces the result. A "Check for updates" button next to "Clear log" lets the user re-trigger manually.

macOS path (`installer::install_macos`): download DMG to `$TMPDIR/movement_logger_update_<pid>`, `hdiutil attach -nobrowse -readonly`, `codesign --verify --deep --strict` the inner `.app`, `ditto` to a sibling staging path (`.MovementLogger.app.new.<pid>`), write a bash helper to `$TMPDIR/movement_logger_install_<pid>.sh` that waits for our PID to die then `mv` swaps the bundles + `open` relaunches, spawn helper detached, send `Done`, sleep 600 ms, `process::exit(0)`. Sparkle-style: avoids the "killed: 9" relaunch failures that happen when you `mv` over a running .app in-process. Linux + Windows + `cargo run` on Mac fall through to an "Open release page" button (banner detects `cfg!(target_os = "macos") && info.dmg_url.is_some() && installer::current_app_bundle().is_some()`).
