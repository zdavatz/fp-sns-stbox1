...
static void send_cmd(uint16_t ogf, uint16_t ocf, uint8_t plen, void *param, uint8_t ext_aci)
{
  ...
  else
  {
    ...
    STM32WB07_06_MEMCPY(payload + 1, &hc, sizeof(hc));
    STM32WB07_06_MEMCPY(payload + HCI_TYPE_SIZE + HCI_COMMAND_EXT_HDR_SIZE, param, plen);
...
/***************************** Exported functions *****************************/
/**
  * @brief  Initializes the peripherals communication with the stm32wb07_06
...
WEAK_FUNCTION(int32_t hci_tl_spi_init(void *pConf))
{
   ...
   */
  STM32WB07_06_PRINTF("hci_tl_spi_init - It is a weak function\r\n");

  return 0;
}

/**
  * @brief  DeInitializes the peripherals communication with the stm32wb07_06
  *         Expansion Board (via SPI, I2C, USART, ...)
  *
  * @param  None
  * @retval int32_t 0
...
WEAK_FUNCTION(int32_t hci_tl_spi_de_init(void))
{
  /* NOTE : This function Should not be modified, when needed,
            the callback could be implemented in the user file
   */
  STM32WB07_06_PRINTF("hci_tl_spi_de_init - It is a weak function\r\n");

  return 0;
}

/**
  * @brief Reset stm32wb07_06 module.
  *
  * @param  None
  * @retval int32_t 0
  */
WEAK_FUNCTION(int32_t hci_tl_spi_reset(void))
{
  /* NOTE : This function Should not be modified, when needed,
            the callback could be implemented in the user file
   */
  STM32WB07_06_PRINTF("hci_tl_spi_reset - It is a weak function\r\n");
...
WEAK_FUNCTION(int32_t hci_tl_spi_send(uint8_t *buffer, uint16_t size))
{
  /* NOTE : This function Should not be modified, when needed,
            the callback could be implemented in the user file
   */
  STM32WB07_06_PRINTF("hci_tl_spi_send - It is a weak function\r\n");

  return 0;
}

/**
  * @brief  Reads from stm32wb07_06 SPI buffer and store data into local buffer.
  *
...
WEAK_FUNCTION(int32_t hci_tl_spi_receive(uint8_t *buffer, uint16_t size))
{
  /* NOTE : This function Should not be modified, when needed,
            the callback could be implemented in the user file
   */
  STM32WB07_06_PRINTF("hci_tl_spi_receive - It is a weak function\r\n");
    ...
    if (hci_hdr->type == HCI_EVENT_PKT)
    {
      ...
      switch (event_pckt->evt)
      {
        case EVT_CMD_STATUS:
          ...
          STM32WB07_06_MEMCPY(req_t->rparam, ptr, req_t->rlen);
          goto done;
        ...
        case EVT_LE_META_EVENT:
          ...
          len -= 1;
          req_t->rlen = MIN(len, req_t->rlen);
          STM32WB07_06_MEMCPY(req_t->rparam, me->data, req_t->rlen);
          goto done;
