...
/**
  ...
  */
uint32_t Command_WriteBufferToFlash(const uint32_t StartAddress, const uint32_t offset, const uint8_t *const pData,
                                    const uint32_t size)
{
  uint32_t ret = 0;
  ...
  if (size % 16)
  {
    STBOX1_PRINTF("Command_WriteBufferToFlash Size=%ld not multiple of 16\r\n", size);
  }
