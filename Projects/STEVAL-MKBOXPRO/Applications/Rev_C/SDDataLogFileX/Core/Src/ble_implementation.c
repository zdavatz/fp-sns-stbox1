...
void init_ble_int_for_blue_nrglp(void)
{
  ...
  /* v239: ENABLE EXTI11 NVIC during init. v238 trace pinned the wedge
   * to hci_reset() — hci_send_req polls an empty rx_queue forever
   * because EXTI11 was masked and the post-send drain in hci_tl_spi_send
   * isn't feeding the queue with the chip's HCI Reset Command Complete.
   * Issue #12 (BLE+SDMMC conflict) is moot under mode-switch (fx_thread
   * is idle in BLE mode, SDMMC quiet). BLEDualProgram works with EXTI11
   * armed throughout init; match that pattern. */
