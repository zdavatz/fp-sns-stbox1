  ...
  *msg_len = mblength + 1;
  ...
  if (ret == NFCTAG_OK)
  {
    ...
    /* Trick to automatically detect the max frame length of the reader
       ...
       before receiving a long response.
    */
...
void ST25FTM_DeviceInit(void)
{
  ...
  if (FinishGood == FINISHA)
  {
    ...
    if (BSP_NFCTAG_WriteConfigIT(STBOX1_MSB_PASSWORD, STBOX1_LSB_PASSWORD,
  ...
  else
  {
    ...
    if (BSP_NFCTAG_WriteConfigIT(STBOX1_MSB_PASSWORD, STBOX1_LSB_PASSWORD,
                                 ...
                                 ST25DVXXKC_GPO1_RFPUTMSG_MASK |
                                 ST25DVXXKC_GPO1_RFGETMSG_MASK) != NFCTAG_OK)
    {
      STBOX1_PRINTF("Error NFCTAG Writing the Interrupt Configuration\r\n");
      STBOX1_Error_Handler(STBOX1_ERROR_NFC, __FILE__, __LINE__);
    }
...
void ST25FTM_UpdateFieldStatus(void)
{
  ...
  if (FinishGood == FINISHA)
  {
    /* first case: Both field transition occurred, need to get the RF state from the register */
...
/**
  ...
  * @details This function reads the interrupt status register from the ST25DV to report which interrupt(s) occurred.
  * @param None
  * @return None.
  */
static void ManageGPO(void)
