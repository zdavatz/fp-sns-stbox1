...
**Issue #15 mode-switch architecture (2026-05-10, in progress)**: BLE and SDMMC will never run concurrently. Boot picks BLE-only (advertising as `STBoxFs`, no logger) or LOG-only (logger active, BLE thread parked) via a TAMP backup register. iPhone connects in BLE mode → writes `OP_START_LOG` (`0x05`) + 4 LE bytes duration → box stores magic + duration + `NVIC_SystemReset` → reboots into LOG mode → fx_thread monitors elapsed time → on expiry clears magic + reset → reboots into BLE mode → app downloads new files via `OP_LIST` / `OP_READ`. Sidesteps issue #12 (BLE+SDMMC concurrency wedge). BLE init still hangs at `hci_reset` response in current builds — see CLAUDE.md "Issue #15 mode-switch architecture" for v218's polling-only approach (EXTI11 NVIC stays masked through entire init, fed via post-send `hci_notify_asynch_evt` drain in `hci_tl_spi_send`).
