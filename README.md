...
The SDDataLogFileX Makefile reads a semver string from a committed `.version` file (currently `0.0.18`) and bakes it in via `-DFW_VERSION_STRING="X.Y.Z"`. **`make` itself does not bump the version** — bump it explicitly with `make bump` (patch +0.0.1), `make bump-minor` (+0.1.0, resets patch) or `make bump-major` (+1.0.0, resets minor+patch) before shipping a release. Output is `build/firmware_v<X.Y.Z>.bin` alongside the unversioned `build/firmware.bin` (the SD-update path needs the exact name). The version is visible in three places: the binary filename, the boot marker in the error log (`fw: v<X.Y.Z> build …`), and the **SD-card root listing itself** — `FW_INFO_v<X.Y.Z>.TXT` is written at boot with old `FW_INFO*` files cleaned up first, so the field tester sees exactly one fingerprint file matching the running firmware without having to open it. See CLAUDE.md ("Build versioning — semver `.version`") for the Make caching gotcha that motivated the `FORCE`-rebuild rule on `app_filex.o`.
