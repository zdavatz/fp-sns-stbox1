...
The `Time` column is the raw `tx_time_get()` value — **ThreadX ticks, 1 tick = 10 ms**. Divide by 100 for wall-clock seconds. Older recordings use the header `Time [mS]` with the same semantics (the misnamed legacy header); current firmware writes `Time [10ms]`. Viz scripts accept both.
...
NMEA line assembly + sentence parsing runs in `gps_thread` (priority 11), not inside the UART4 RX ISR. The IRQ just pushes incoming bytes into a 1 KB ring buffer (`RxRing[]` in `gps_nmea.c`) and the thread calls `GPS_Process()` once per poll cycle to drain it. Before this split, parsing ran in the UART4 IRQ at NVIC priority 6 and preempted the SDMMC1 IRQ (priority 14); at 10 Hz GPS that starved the SD writer enough to drop the sensor rate from 100 Hz to ~7.7 Hz after ~90 s. Now the IRQ path is O(1) per byte and SD transfers are uninterrupted. Validated post-fix: sustained 100 Hz sensor + 10 Hz GPS across 13.8 min with no Δtick > 1 runs anywhere in the file.
...
- **Red LED blinking continuously** — fatal error (`Error_Handler`), details in `Error_Log_Pump_Tsueri_dd.mm.yyyy.log` on SD
...
- **Red LED morse pattern (N blinks → 1.2 s pause → repeat)** — `tx_application_define` halt before `tx_kernel_enter`. N = 2: tx byte-pool create failed; 4: `App_ThreadX_Init` failed (BLE thread alloc/create — usually `TX_APP_MEM_POOL_SIZE` too small); 6: fx byte-pool create failed; 8: `MX_FileX_Init` failed. Distinguishable from the continuous `Error_Handler` blink by the rhythm.

Audible boot signals via the piezo buzzer (PE13 / TIM1_CH3): two ascending tones (1500 Hz / 3000 Hz, ~240 ms total) right before ThreadX takes over — confirms every pre-RTOS init step succeeded; louder/clearer than the 3-blink visual sequence in sunlight. Three quick 2500 Hz chirps (~270 ms) on the first valid GPS fix.
...
The SDDataLogFileX firmware advertises as **`PumpTsueri`** with PIN-secure pairing (PIN `123456`) and exposes a tiny custom GATT service for downloading recorded files without removing the SD card. Two characteristics live under the BlueST features service (`00000000-0001-11e1-9ab4-0002a5d5c51b`):
...
The feature is gated on `STBOX1_ENABLE_BLE_SYNC 1` in `Core/inc/stbox1_config.h` (default on). Set it to `0` to remove the entire BLE stack and reclaim ~28 KB of flash. The no-BLE build is also useful as a diagnostic when a fresh box's boot blink sequence completes (1/2/3 green) but the LED then goes dark with both an empty SD card *and* no `PumpTsueri` advertisement — see CLAUDE.md ("BLE EXTI11 IRQ-storm fix + ThreadX byte-pool sizing") for the full rationale. The first reported case of that symptom was traced to `TX_APP_MEM_POOL_SIZE` being too small for the BLE thread's stack; the pool has been bumped twice (1 KB → 8 KB → 16 KB in `AZURE_RTOS/App/app_azure_rtos_config.h`) to leave comfortable margin against `tx_byte_pool` internal headers + alignment padding.
...
A third refinement catches a half-dead BlueNRG-LP that ACK's SPI bytes but then hangs the kernel inside `init_ble_manager`'s GATT setup. The new two-stage chip-alive probe in `ble_sync.c::ble_chip_alive_probe()` requires both an SPI ACK *and* a Command Complete response within 500 ms before calling `bluetooth_init`. The probe outcome is recorded in `g_ble_probe_status` and snapshotted into the error log via `ErrorLog_Open` so post-session you can tell whether BLE was up or skipped. Probe + the original three-part fix combined are not sufficient on the 3.3V-hardware-modded SensorTile.box PRO reservebox — even with `STBOX1_ENABLE_BLE_SYNC 0` the same hardware shows non-deterministic SDMMC hangs at random `fx_file_*` operations, suggesting the modded power supply has signal/integrity issues unrelated to BLE. The hang has been progressively localized through v10–v13 field-test builds: lowering the SDMMC kernel clock from HSPEED to NSPEED (`ea4bd38a`) wasn't enough; reformatting the SD card to a clean FAT32 wasn't enough either (ruling out FAT corruption from prior bad writes); v13's bisect pinned it to `fx_media_flush` specifically — the *first* per-line flush after `ErrorLog_Open`'s big-batch flush hangs. The current mitigation is to stop calling `fx_media_flush` after every line and use the same many-writes-one-flush pattern that `ErrorLog_Open` itself uses successfully. See CLAUDE.md ("Two-stage BLE chip-alive probe" / "Per-step ErrorLog markers" / "Batched-flush write pattern") and issue #12 for the full diagnostic timeline.

**Issue #15 mode-switch architecture (2026-05-11, working end-to-end)**: BLE and SDMMC never run concurrently. Boot picks BLE-only (advertising as `PumpTsueri`, no logger) or LOG-only (logger active, BLE thread parked) via a TAMP backup register. Host connects in BLE mode → writes `OP_START_LOG` (`0x05`) + 4 LE bytes duration → box stores magic + duration + `NVIC_SystemReset` → reboots into LOG mode → fx_thread monitors elapsed time → on expiry clears magic + reset → reboots into BLE mode → app downloads new files via `OP_LIST` / `OP_READ`. Sidesteps issue #12 (BLE+SDMMC concurrency wedge). The user button is the fallback trigger for a fixed 5-minute session via the same TAMP-magic + reset path; in LOG mode the button aborts a session early. Unblocked by commit `96c6f829` (added the missing `EXTI11_IRQHandler` strong override — see CLAUDE.md "BLE bring-up — current state"). Verified `bluetoothctl scan le` shows `PumpTsueri`, nRF Connect on iPhone + MovementLogger GUI both LIST/READ/START_LOG end-to-end.
...
The speed panel uses an **acceleration-gated** position-derived speed (haversine on GPS lat/lon deltas). Samples where `|Δspeed|/Δt > 15 km/h/s` (≈4 m/s²) and `v > 15 km/h` are rejected as multipath-induced position jumps before the 5 s rolling median. SUPfoil paddle strokes produce 1–3 m/s², anything above ~4 m/s² implies a ≥4 m position jump inside the module's ~5–10 m horizontal-error envelope. Real-data validation: raw max 83.7 km/h → after gate 29.5 km/h → after smoothing 17.6 km/h. Without the gate the 5 s median alone couldn't suppress 30 km/h multipath spikes.
...
- **`MovementLogger`** — cross-platform (Win/Mac/Linux) drag-and-drop GUI front-end for `stbox-viz animate`, with a built-in **BLE FileSync** panel (v0.1.4+) that scans for `PumpTsueri`, lists SD-card files over BLE and downloads them straight into the form's drop zone — no card swap. Files are split into **Sensor** (Sens/Gps/Bat/Mic) and **Debug** (FW_INFO, CHK, error log, macOS pollution, phantoms) groups with per-group "Tick all" toggles; only Sensor rows default-ticked. Each row has a trash button (🗑) wired to the firmware's `OP_DELETE`. Downloads run serially with a per-row progress bar; finished rows get a green ✅ "downloaded" badge. The log panel is fully selectable (Cmd-A / Cmd-C, plus a "Copy all" button) and grows with the window. Disconnect transitions the UI to Idle immediately so Scan is usable while btleplug is still tearing the link down; close via Cmd-Q for a clean disconnect (eframe `on_exit` sends LL_TERMINATE_IND before the process exits — otherwise the box's BLE link stays occupied until the supervision timeout ~10–30 s, or a box reboot). Source at `Utilities/rust/stbox-viz-gui/` — see [its README](Utilities/rust/stbox-viz-gui/README.md). Tagged releases (`vX.Y.Z`) produce binaries for Linux x86_64, macOS Apple Silicon (with a `.app` bundle and a fully-notarized + stapled DMG — works offline from v0.1.6 onwards) and Windows x86_64 via `.github/workflows/release.yml`. Intel Mac was dropped after v0.1.5; v0.1.5 and earlier DMGs are also unsigned (no Apple secrets were configured on the repo before v0.1.6) — start from v0.1.6 if Gatekeeper rejects an older download. From v0.1.9 onwards the app checks GitHub Releases on startup and surfaces an "Update available" banner. From v0.1.10 onwards clicking **Update now** does the full Firefox-style in-place swap on **all three platforms** — macOS downloads the notarized DMG and swaps the `.app`, Linux extracts the tar.gz and swaps both binaries via a bash helper, Windows extracts the zip and swaps both `.exe`s via a PowerShell helper. In every case the helper waits for the running process to exit, swaps the artifacts, and relaunches the new version.
