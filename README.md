...
**Issue #15 mode-switch architecture (2026-05-10, in progress)**: BLE and SDMMC will never run concurrently. Boot picks BLE-only (advertising as `STBoxFs`, no logger) or LOG-only (logger active, BLE thread parked) via a TAMP backup register. iPhone connects in BLE mode → writes `OP_START_LOG` (`0x05`) + 4 LE bytes duration → box stores magic + duration + `NVIC_SystemReset` → reboots into LOG mode → fx_thread monitors elapsed time → on expiry clears magic + reset → reboots into BLE mode → app downloads new files via `OP_LIST` / `OP_READ`. Sidesteps issue #12 (BLE+SDMMC concurrency wedge). User button is **ignored in BLE mode** so sessions are strictly GUI-driven; in LOG mode the button still aborts a session early. BLE init still hangs **inside `hci_init`** in current builds — v222 (current uncommitted firmware) replaces a suspect `printf` with direct `UsbCdc_Write` to triangulate whether newlib state is corrupted by mid-init. See CLAUDE.md "Issue #15 mode-switch architecture" for the v218→v222 progression and the next-session triangulation matrix.
